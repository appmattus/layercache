...
class EncryptionEncryptConnectedTest {
    ...
    fun encrypts_and_decrypts_with_keys_reloaded() {
        runBlocking {
            ...
            assertEquals("hello", cache.encrypt(appContext).get("key"))
        }
    }
    ...
    @Test
    fun key_is_encrypted_in_backing_map() {
        runBlocking {
            // when we set a value
            encrypted.set("key", "hello")

            // then the key does not appear in the backing map
