...
internal object KeyProperties {
    ...
    val ENCRYPTION_PADDING_NONE =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) KeyProperties.ENCRYPTION_PADDING_NONE else "NoPadding"
    val ENCRYPTION_PADDING_PKCS7 =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) KeyProperties.ENCRYPTION_PADDING_PKCS7 else "PKCS7Padding"
    val ENCRYPTION_PADDING_RSA_OAEP =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) KeyProperties.ENCRYPTION_PADDING_RSA_OAEP else "OAEPPadding"
    val ENCRYPTION_PADDING_RSA_PKCS1 =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) KeyProperties.ENCRYPTION_PADDING_RSA_PKCS1 else "PKCS1Padding"
